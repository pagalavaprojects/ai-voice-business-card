// src/app/api/admin/cms/seo/route.ts
...
export async function GET(req: NextRequest) {
  try {
    ...
    const seo = await cmsRepository.getSEOSettings(companyId);
    return formatApiResponse(seo, 200, "SEO settings retrieved successfully");
  } catch (error: unknown) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const updated = await cmsRepository.upsertSEOSettings({ ...body, company_id: companyId });
    return formatApiResponse(updated, 200, "SEO settings updated successfully");
  } catch (error: unknown) {
    return handleApiError(error);
  }
}
